    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (response.data.role !== 'MASTER_ADMIN') {
                toast.update(toastId, { render: 'Not authorized as Master Admin', type: "error", isLoading: false, autoClose: 3000 });
                setIsLoading(false);
                return;
            }
            ...
            <div className="bg-white/60 dark:bg-slate-900/70 backdrop-blur-xl p-10 rounded-3xl shadow-[0_8px_30px_#0000001a] dark:shadow-2xl w-full max-w-md relative z-10 mx-4 border border-white/60 dark:border-white/10 transition-colors duration-500">
                ...
                <form onSubmit={handleLogin} className="space-y-6">
                    ...
                    <button
                        type="submit"
                        disabled={isLoading}
                        className="w-full bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-xl py-3.5 font-bold tracking-wide hover:shadow-lg hover:shadow-purple-500/30 hover:scale-[1.02] transform transition-all active:scale-[0.98] disabled:opacity-70 disabled:hover:scale-100 flex justify-center items-center gap-2"
                    >
                        {isLoading ? (
                            <>
